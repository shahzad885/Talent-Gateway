...
const AlumniChatList = () => {
  ...
  const fetchChats = async () => {
    try {
      const response = await api.get("/alumni/chats");
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const response = await api.post(
        `/alumni/chats/${activeChat._id}/messages`,
        {
          content: newMessage,
        }
      );
  ...
  const handleChatSelect = async (chat) => {
    try {
      const response = await api.get(`/alumni/chats/${chat._id}`);
  ...
  const markMessagesAsRead = async (chatId) => {
    try {
      await api.post(`/alumni/chats/${chatId}/read`);

      // Emit read receipt through socket
      if (socketRef.current && isConnected) {
            ...
            {chats.length === 0 ? (
              <div className="text-center py-16 px-6">
                ...
                <p className="text-sm text-gray-500">
                  Start a chat with students from their profiles
                </p>
              </div>
            ) : (
                    ...
                    >
                      <div className="flex items-center mb-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-gray-500">
                            {otherParticipant.role === "student" ? "Student" : "Alumni"}
                          </p>
                        </div>
                        {unreadCount > 0 && (
                          <span 
                        ...
                        <div>
                          <p className="text-sm line-clamp-2 text-gray-600">
                            <span className="font-medium">
                              {lastMessage.sender === user._id ? "You: " : `${otherParticipant.name}: `}
                            </span>
                            {lastMessage.content}
                          </p>
